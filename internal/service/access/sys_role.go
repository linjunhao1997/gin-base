...
func GetApiResources(roleId ...int) ([]model.SysResource, error) {
	...
	err := db.DB.Where("type = ? and disable = 0", model.API).Find(&apis).Error
	if err != nil {
		return apis, err
	}
	return apis, nil
...
func RelatedRoleResources(roleId int, resourceIds []int) error {
	...
	return db.DB.Model(&role).Association("SysResources").Replace(&resources)
}
...
func GetSysResources(roleId ...int) ([]*model.SysResource, error) {
	...
	if err := db.DB.Distinct().Table("sys_resource").Joins("left join sys_role_r_sys_resource on sys_resource.id = sys_role_r_sys_resource.sys_resource_id").Where("sys_role_r_sys_resource.sys_role_id IN ? and sys_resource.type = ?", roleId, model.MODULE).Preload(slice[3]).Find(&sysResource).Error; err != nil {
